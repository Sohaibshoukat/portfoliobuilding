...
const CV = require("../Schema/WritterSchema");
...
//Store new notes
router.post("/AddCVData", fetchuser, upload.fields([{ name: 'image', maxCount: 1 }, { name: 'images', maxCount: 4 }]), async (req, res) => {
    try {
        const image = req.file.buffer.toString('base64');
        const images = req.files['images'].map(file => ({ Image: file.buffer.toString('base64') }));
...
router.get("/SingeCV/:id", fetchuser, async (req, res) => {
    try {
