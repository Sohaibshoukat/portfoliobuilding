...
function MyResume() {
    ...
    const selectedTemplate = useSelector(state => state.WrittenStore.selectedTemplate)
    ...
    const SaveData = async () => {
        if(localStorage.getItem('editItem')){
            try {
                const response = await fetch(`http://localhost:5000/api/writter/DeleteCV/${localStorage.getItem('editItem')}`, {
                    method: 'DELETE',
                    headers: {
        ...
        formData.append('Template', "Writter");
        formData.append('FirstName', dataStore.firstName);
        formData.append('LastName', dataStore.lastName);
        formData.append('Email', dataStore.Email);
        formData.append('Mobile', dataStore.Mobile);
        formData.append('Web', dataStore.Web);
        formData.append('Facebook', dataStore.Facebook);
        formData.append('Linkdin', dataStore.Linkdin);
        formData.append('Twitter', dataStore.Twitter);
        ...
        const apiUrl = 'http://localhost:5000/api/writter/AddCVData';
