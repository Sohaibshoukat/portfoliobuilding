...
import { templateImagesPaths, fieldTemplate } from '../Data/Data'//templateImagesPaths is imported from Data.js which is used to display static images of various templates on the Home page.
import { useDispatch } from 'react-redux'
import { updateState } from '../../ReduxManager/dataStoreSlice'
import { updateStateweb } from '../../ReduxManager/WebDataStore'
import { updateWrittenState } from '../../ReduxManager/WritterDataStore'
    ...
    return (
        <div style={{ minWidth: '300px' }}>
            ...
            <div className='container' style={{ color: '#1f4287', }}>
                ...
                <div className='row'>
                    {fieldTemplate.map((currentTemplate, index) => {
                        return (
                            <div className='col col-lg-3 col-md-6  col-12 mt-5' key={shortid.generate()}>
                                <div
                                    style={{ position: 'relative' }}
                                ...
                                >
                                    ...
                                    {isMouseOver === currentTemplate.name           //this conditional rendering is showing 'useTemplate' button when isMouseOver === currentTemplate.name //
                                        ? (<>
                                            {index == 0 ? (
                                                <Link to="/fillingWeb/personalinfo">
                                                    <button className='btn btn-primary'
                                                        style={{ position: 'absolute', top: '50%', right: '30%', }}
                                                        onClick={() => {
                                                            dispatch(updateStateweb({  //this dispatch function is used to update value of 'selectedTemplate' with the targetedTemplate in dataStoreSlice.js//
                                                                key: 'selectedTemplate',
                                                                value: currentTemplate.name
                                                            }))
                                                        }}
